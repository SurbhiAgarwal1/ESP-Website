...
def top_classes(request, tl, program, instance):
    ...
    if tl == 'manage':
        ...
        favorite_questions = survey.questions.filter(question_type__name='Favorite Class')

        if len(rating_questions) < 1 and len(favorite_questions) < 1:
            raise ESPError('Couldn\'t find an "overall rating" or "Favorite Class" question in this survey.', log=False)

        # Determine which type of question we're working with
        use_favorite_class = len(rating_questions) < 1 and len(favorite_questions) >= 1
        ...
        if use_favorite_class:
            ...
            for cls in classes:
                ...
                class_data = {
                    'class': cls,
                    'numratings': votes,
                    'avg': votes,  # For favorite class, "avg" is the vote count
            ...
            context = {
                ...
                'rating_cut': 0,  # Not applicable for favorite class
                'num_cut': num_cut,
